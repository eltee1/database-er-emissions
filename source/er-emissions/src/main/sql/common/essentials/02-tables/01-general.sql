 ...
 * - de checksum van de tabel na een eventuele wijziging (@column checksum_change), wordt on demand gevuld door de functies checks.checksum_metadata() en checks.checksum_public_tables(). Middels deze
...
CREATE TABLE checks.metadata (
	...
	remarks text NULL,
	checksum_check boolean GENERATED ALWAYS AS (coalesce(checksum_import::text, '') = coalesce(checksum_change::text,'')) STORED,
